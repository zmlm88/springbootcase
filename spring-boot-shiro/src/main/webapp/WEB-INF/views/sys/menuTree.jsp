			...
			<div class="accordion-group">
				<div class="accordion-heading">
					<a class="accordion-toggle" data-toggle="collapse" data-parent="#menu-${menu.tMenu.parentid}"
						data-href="#collapse-${menu.tMenu.id}" href="#collapse-${menu.tMenu.id}" title="${menu.tMenu.name}"><i
						class="icon-chevron-${not empty firstMenu && firstMenu ? 'down' : 'right'}"></i>&nbsp;${menu.tMenu.name}</a>
				</div>
				<div id="collapse-${menu.tMenu.id}"
					class="accordion-body collapse ${not empty firstMenu && firstMenu ? 'in' : ''}">
					<div class="accordion-inner">
						<ul class="nav nav-list">
							<c:forEach items="${menu.subList}" var="menu2">
								<c:if test="${menu2.tMenu.parentid eq menu.tMenu.id && menu2.tMenu.isshow == 1}">
									<li><a data-href=".menu3-${menu2.tMenu.id}"
										href="${fn:indexOf(menu2.tMenu.href, '://') eq -1 ? ctx : ''}${not empty menu2.tMenu.href ? menu2.tMenu.href : '/404'}"
										target="${not empty menu2.tMenu.target ? menu2.tMenu.target : 'mainFrame'}"><i
											class="icon-${not empty menu2.tMenu.icon ? menu2.tMenu.icon : 'circle-arrow-right'}"></i>&nbsp;${menu2.tMenu.name}</a>
										<%--
