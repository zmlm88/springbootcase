...
<head>
	...
	<script type="text/javascript">
		...
		$(document).ready(function(){
			...
			var zNodes=[
			         {id:0, pId:0, name:"资源列表"},
		            <c:forEach items="${userMenuList}" var="tpl">
		            {id:'${tpl.id}', pId:0, name:"${tpl.name}", url:"${ctx}/web/sys/user/menu/list/${tpl.id}", target:"userMenuFrameList"},
		            </c:forEach>];
			...
			if(node){
				...
				var url=node.url;
				$(window.parent.document).find("#userMenuFrameList").attr("src",url); 
			}
			tree.checkNode(node);
		...
		function addForward(){
			$(window.parent.document).find("#userMenuFrameList").attr("src","<c:url value='/web/sys/user/menu/rootMenu?rootMenuOper=ADD' />"); 
		}
		function modify(){
		...
		function remove(){
			...
			if(nodes.length >0){
				var roleId =nodes[0].id;
				var url ='<c:url value="/web/sys/user/menu/deleteRootMenu/" />'+roleId;
				var delTxt = '要删除资源:'+nodes[0].name+'吗？';
...
<body>
	...
	<div class="accordion-group">
		<div id="left" class="accordion-group">
			<div class="accordion-heading">
		    	<a class="accordion-toggle">资源 &nbsp;&nbsp;&nbsp;&nbsp; &nbsp;&nbsp;&nbsp;&nbsp; &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
		    		<i class="icon-refresh" onclick="refreshTree();"></i>
		    		<i class="icon-remove" id="remove" alt="刪除资源" ></i>
		    		<i class="icon-plus" id="add" alt="添加资源" ></i>
		    		<i class="icon-edit" id="modify" alt="编辑资源" ></i>
	...
	<nav class="navbar navbar-default navbar-fixed-bottom">
		<div class="container-fluid margin_bottom_5 margin_top_5 clear_padding text-right">
